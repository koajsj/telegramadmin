...
- 管理员同步：后台定时拉取各群管理员状态，给权限判断提供辅助缓存。
...
- 自动任务：
  - 管理员同步任务：定时刷新各群管理员缓存。
  - 自动学习巡检：定时扫描历史违规并产出候选建议。
  - 禁言释放任务：定时解除到期禁言并写入审计日志。
- Inline 按钮（日志群快捷处置）：
  - `警告（记录违规）`：写入处罚记录，不禁言。
  - `禁言10分钟（短期）`：快速短时禁言。
  - `禁言1小时（中期）`：快速中时禁言。
...
7. 启动前做数据库与 Redis 连通性检查，失败会直接退出，不会带病运行
...
- 管理员缓存：`chat_admins` 由后台同步任务维护；同步失败时会保留旧缓存，不会清空已有数据。
...
- 权限控制、配置解析、缓存与迁移烟雾测试
- 依赖与启动前校验：`python -m pip check`、数据库/Redis 连通性检查
...
- 补充：当前实现会在启动阶段先检查 PostgreSQL 和 Redis，若环境未就绪会直接退出并输出明确错误，避免把问题延迟到按钮点击阶段。
